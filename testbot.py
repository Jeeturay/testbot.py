...
@client.command(pass_context=True)
async def buy():
           await client.say('Hello '+climate.user.name+'**Thanks for using /buy command** , **__BUT YOU CAN ONLY BUY POINTS IN OUR OFFICIAL SERVER__** **if u are already in your official lives store server then proceed your buying process by typing `/buy points`**   *** ( type `/help` for more information)***.')
@client.command(pass_context=True)
async def instant():
